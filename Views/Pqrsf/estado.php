<div class="card">
    <div class="header">PQRSF <?=strtoupper($_REQUEST['estado']);?></div>
    <div class="body">
        <div class="responsive">
            <table id="tablePqrs" class="table table-bordered table-responsive">
                <thead>
                    <tr>
                        <th WIDTH="10%">Tipo</th>
                ...
                <tbody>
                    <?php foreach ($pqrsByest as $value) : ?>
                        <tr>
                            <td>
                                <?= ucwords($value->tipo_peticion) . '<br> <span class="badge bg-orange"> ' . ucwords($value->estado) . '</strong> </span>
                                <br>'. $value->radicado  ?>
                                <?= $value->estado == 'asignado' ? '<br> <span class="badge bg-blue"> A: ' . ucwords($value->responsable) . '</span>' : ''; ?>
                            </td>
                            <td><?= $value->fecha_registro;
                                ...
                                // will output 2 days
                                // echo $diff->days . ' dias ';
                                echo $value->dias_transcurridos==0 ? '<br>' . ($diff->invert == 1) ? '<br>' . $diff->days . ' Dias '  : $diff->days . ' Dias ': '<br>' . $value->dias_transcurridos.' Dias.';
                                ?></td>
                            <td style="text-align: justify;"><?=  $value->descripcion   . '<br> <span class="badge bg-blue"> ' . ucwords($value->nombres . ' ' . $value->apellidos) . '</span><br> <strong> Número contacto: ' . $value->n_contacto . '<br> Correo: ' . $value->email . '</strong>' ?></td>
                            <td style="vertical-align: middle;text-align: center;">
                                ...
                                switch ($value->estado) {
                                    ...
                                    case 'asignado': ?>
                                        ...
                                        <a onclick="Soporte('<?= $value->id ?>')" data-toggle="modal" href='#modal-id' data-toggle="tooltip" data-placement="bottom" title="Registra la respuesta para la adjuntar Soporte">
                                            <span class="material-icons">
                                                attachment
                                            </span>
                                        </a>
